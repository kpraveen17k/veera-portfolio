document.addEventListener("DOMContentLoaded", () => {
  ...
  // 2. Fetch & Render Real Experience (Zero Hardcoded Dummy Records)
  let currentExpList = [];
  ...
  async function renderExperience() {
    ...
    try {
      if (typeof firebase !== "undefined" && firebase.apps.length) {
        const snap = await firebase.firestore().collection("experience").get();
        snap.forEach((doc) => cloudExp.push({ id: doc.id, ...doc.data() }));
      }
    } catch (err) {
    ...
    let localExp = [];
    try {
      localExp = JSON.parse(localStorage.getItem("veera_custom_experience") || "[]");
    } catch {
      localExp = [];
    }

    // Combine Unique User-Added Experience
    const uniqueIds = new Set();
    currentExpList = [...cloudExp, ...localExp].filter((item) => {
    ...
    if (currentExpList.length === 0) {
      grid.innerHTML = `
        <div style="grid-column: 1/-1; text-align: center; padding: 50px 20px; background: #ffffff; border-radius: 16px; border: 1px dashed #cbd5e1;">
          ...
          <h3 style="font-size: 15px; font-weight: 800; color: #1e293b;">No Experience Added Yet</h3>
          <p style="font-size: 12px; color: #64748b; margin-top: 4px;">Add your work history, company roles, and milestones from Admin Hub.</p>
    ...
    // Render Cards for Uploaded Experience (3-Line Clamping + View Details Button)
    currentExpList.forEach((exp) => {
      const bannerImg = (exp.images && exp.images[0]) || exp.imageUrl || "images/veera.png";
      const card = document.createElement("div");
      card.className = "exp-box";
      card.innerHTML = `
        <div class="exp-thumb-wrap">
          <img src="${bannerImg}" alt="${exp.title}" onerror="this.src='images/veera.png';" />
          <span class="badge-status">${exp.duration || "Present"}</span>
        </div>
        <div class="exp-box-body">
          <h3>${exp.title}</h3>
          <h4 class="company-sub">${exp.company || "Organization"}</h4>
          <!-- 3-line clamped description -->
          <p class="clamped-desc">${exp.desc || "Verified professional role highlights."}</p>
          <div class="exp-card-actions">
            <button type="button" class="btn-view-details" onclick="window.openExpDetails('${exp.id}')">
    ...
    updateCounters(currentExpList.length);
  }

  function updateCounters(count) {
    const totalExpEl = document.getElementById("statTotalExpCount");
    const totalProjEl = document.getElementById("statTotalExpProjects");

    if (totalExpEl) totalExpEl.innerText = count;

    // Fetch projects count
    let projCount = 0;
    try {
      const projList = JSON.parse(localStorage.getItem("veera_custom_projects") || "[]");
      projCount = projList.length;
    } catch {}
    if (totalProjEl) totalProjEl.innerText = projCount > 0 ? `${projCount}+` : count;
  }

  // 3. Multi-Image Showcase Modal Controller
  const modal = document.getElementById("expInfoModal");
  ...
  window.openExpDetails = function(id) {
    const item = currentExpList.find((e) => e.id === id);
    if (!item || !modal) return;

    document.getElementById("modalExpTitle").innerText = item.title;
    document.getElementById("modalExpBadge").innerText = item.duration || "Timeline";
    document.getElementById("modalExpCompany").innerHTML = `<i class="fa-solid fa-building"></i> ${item.company || "Organization"}`;
    document.getElementById("modalFullDescription").innerText = item.desc || "No detailed highlights provided.";
  ...
  const closeModalBtn = document.getElementById("closeExpModalBtn");
  if (closeModalBtn) {
  ...
  if (searchInput) {
    searchInput.addEventListener("input", (e) => {
      ...
      document.querySelectorAll(".exp-box").forEach((card) => {
